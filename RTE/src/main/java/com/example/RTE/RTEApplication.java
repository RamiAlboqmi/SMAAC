...
public class RTEApplication {
	...
	public static void main(String[] args) {
		...
		System.out.println("RTE v4.0 with new STIX and APG v6 and CH v1 (Journal paper)");
	    ...
	    int timeToExecuteTheBaselinePhase = Integer.parseInt(System.getenv().getOrDefault("thresholdTimeInMins", "1"));
        ...
        ScheduledFuture<?> countdown = sch.schedule(new Runnable() {
            @Override
            public void run() {
        	    System.out.println("Trying to call getAllServiceNameFromTISTemp()");
            ...
            //	try {
            		// for the journal paper. for mow no need to got the telemetry data.
            		// So disable the below function (rest.ACS_baselinePhase();). Then call a temp fun to call TIS to get the below:
            			// 1) service names 
            	try {
            		
					System.out.println(rest.ACS_baselinePhase());
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
            			// 2) Their intial TM 
            			// 3) then RTE perform recall of TIS continous TM assessment after I do patching. 
        ...
        System.out.println("RTE Processing Time: start_time:" + start_time);
        System.out.println("RTE Processing Time: end_time:" + end_time);
        System.out.println("Processing Time: " + (end_time - start_time) / 1_000_000 + " ms");
        sch.shutdown();
	}
